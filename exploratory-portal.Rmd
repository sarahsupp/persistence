...
#for NSF proposal example, controls only
# Reorder following the value of another column:
test %>%
  filter(treatment == "control") %>%
  mutate(species = fct_reorder(species, prop)) %>%
  ggplot(aes(as.numeric(year), species)) + 
  geom_point(aes(col=as.factor(traj)), shape=15, size=5) + 
  geom_tile(col="white", aes(fill=as.factor(incidence))) + 
...
ggsave("portal-incidence.png", height=5, width=5)
```
